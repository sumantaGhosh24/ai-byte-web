...
const UpdateCoursePage = () => {
  ...
  useEffect(() => {
    if (course?.course?.category) {
      ...
      setPreview(course?.course?.thumbnailUrl ?? null);
    }
  }, [course, form]);
  ...
  const onSubmit = (values: CourseFormValues) => {
    ...
    if (file) {
      if (course?.course?.thumbnailPublicId) {
        deleteImage.mutate(
          { public_id: course?.course?.thumbnailPublicId },
          {
      ...
      updateCourse.mutate(
        { ...values, courseId: course?.course?.id },
        {
          onError: (error) => toast.error(error.message),
          onSuccess: (data) => {
            form.reset();
    ...
    <div className="my-20">
      <Card className="border-border/50 shadow-sm">
        <CardHeader className="space-y-3">
          ...
          <CardDescription className="mt-1">Update a course with all settings.</CardDescription>
        </CardHeader>
        <Separator />
        <CardContent className="pt-6">
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <FieldGroup>
              ...
              <Button
                type="submit"
                disabled={updateCourse.isPending || uploadImage.isPending}
                className="min-w-40"
              >
                {updateCourse.isPending || uploadImage.isPending ? (
                  <>
